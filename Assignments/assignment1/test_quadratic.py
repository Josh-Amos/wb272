...
def output_capture():
    ...
    captured_outs = [cStringIO.StringIO(), cStringIO.StringIO()]
...
def check_results(qsolve, coeffts, unique=True):
    with nostdout():
        results = qsolve(*coeffts)
    if not hasattr(results, '__iter__') or len(results) != 2:
        raise TestError('function does not return 2 values')
    if not reduce(lambda x, y: x and y, [isinstance(z, float) for z in
                                         results], True):
        raise TestError('function does not return all floats')
    if unique and len(results) != len(set(results)):
        raise TestError('roots must be unique')
    a, b, c = coeffts
    return reduce(lambda x, y : x and y,
                  [are_close(a*x*x + b*x + c, 0.0) for x in results])

### test cases #################################################################

def test01(qsolve):
    """qsolve(1, 0, 0)"""
    return check_results(qsolve, (1, 0, 0), unique=False)

def test02(qsolve):
    """qsolve(-1, -2, 0)"""
    return check_results(qsolve, (-1, -2, 0))

def test03(qsolve):
    """qsolve(1, 6, 5)"""
    return check_results(qsolve, (1, 6, 5))

def test04(qsolve):
    """qsolve(1, -2, 1)"""
    return check_results(qsolve, (1, -2, 1), unique=False)

def test05(qsolve):
    """qsolve(1, -2, -4)"""
    return check_results(qsolve, (1, -2, -4))

def test06(qsolve):
    """qsolve(-1.3, 2.7, 3.1)"""
    return check_results(qsolve, (-1.3, 2.7, 3.1))
...
def set_up_tests():
    mts = ModuleTestSetup('quadratic.py')
    fts = FunctionTestSetup(mts, 'qsolve', 'test')
    fts.set_illegal_statements(['If', 'Import', 'ImportFrom', 'For', 'Slice',
        'While'])
