...
def run_test(f, *params):
    with output_capture() as outs:
        rval = f(*params)
    if rval != None:
        raise TestError('Why did your function return something?')
    return '\n'.join(line.rstrip() for line in outs[0].split('\n'))

### test cases #################################################################

def test01(diamond):
    """diamond('I')"""
    return run_test(diamond, 'I'), '''\
I
'''

def test02(diamond):
    """diamond('OF')"""
    return run_test(diamond, 'OF'), '''\
OF
'''

def test03(diamond):
    """diamond('AIR')"""
    return run_test(diamond, 'AIR'), '''\
 I
AIR
 I
'''
...
def test06(diamond):
    """diamond('ENTROPY')"""
    return run_test(diamond, 'ENTROPY'), '''\
   R
  TRO
 NTROP
ENTROPY
...
def test07(diamond):
    """diamond('ENTHALPY')"""
    return run_test(diamond, 'ENTHALPY'), '''\
   HA
  THAL
 NTHALP
ENTHALPY
...
def test08(diamond):
    """diamond('supercalifragilisticexpialidocious')"""
    return run_test(diamond, 'supercalifragilisticexpialidocious'), '''\
                st
               isti
              listic
             ilistice
         ...
         fragilisticexpia
        ifragilisticexpial
       lifragilisticexpiali
      alifragilisticexpialid
     califragilisticexpialido
    rcalifragilisticexpialidoc
   ercalifragilisticexpialidoci
  percalifragilisticexpialidocio
...
def set_up_tests():
    mts = ModuleTestSetup('word.py')
    fts = FunctionTestSetup(mts, 'diamond', 'test')
    return mts
