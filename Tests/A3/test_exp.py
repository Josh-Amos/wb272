...
def test01(exponential):
    """exponential(0.01)"""
    return run_test(exponential, 0.01), math.exp(0.01)

def test02(exponential):
    """exponential(1)"""
    return run_test(exponential, 1), math.exp(1)

def test03(exponential):
    """exponential(3)"""
    return run_test(exponential, 3), math.exp(3)

def test04(exponential):
    """exponential(0.1)"""
    return run_test(exponential, 0.1), math.exp(0.1)

def test05(exponential):
    """exponential(77)"""
    return run_test(exponential, 77), math.exp(77)

def test06(exponential):
    """exponential(math.pi)"""
    return run_test(exponential, math.pi), math.exp(math.pi)

def test07(exponential):
    """exponential(13)"""
    return math.log(run_test(exponential, 13)), 13
...
def has_legal_source(fts):
    ...
    if len(loops & checker.get_node_types()) > 0:
        msg = 'WARNING: used for loop ... fixing the number of terms?'
        print(ANSI_COLOR['yellow'](msg))
    ...
    for name, test in fts.tests:
        try:
            ...
            if abs(rvalue - expected) < EPSILON:
                ...
                print('{}returned {} instead of a number close to {}'.format(
                        ' '*8,
...
def set_up_tests():
    mts = ModuleTestSetup('exp.py')
    fts = FunctionTestSetup(mts, 'exponential', 'test')
